...
namespace PoliticiansAndParties.Api.Test.Integration
{
    ...
    {
        static Faker<PoliticalPartyDto> _partyGenerator = new Faker<PoliticalPartyDto>()
            .RuleFor(x => x.Name, f => f.Company.CompanyName())
            .RuleFor(x => x.ImageUrl, f => f.Image.PicsumUrl())
            .RuleFor(x => x.Tags, f => f.Lorem.Words(new Random().Next(1, 10)).ToHashSet())
            .RuleFor(x => x.Id, Guid.NewGuid());

        static Faker<PoliticianDto> _politicianGenerator = new Faker<PoliticianDto>()
        ...
        public static IEnumerable<PoliticalPartyDto> GeneratePoliticalParties(int count = 10, int maxPoliticiansInParty = 20)
        {
            var parties = _partyGenerator.Generate(count);

            parties.ForEach(x => x.Politicians = _politicianGenerator.GenerateBetween(1, maxPoliticiansInParty));
        ...
        public static PoliticianDto GeneratePolitician()
        {
            return _politicianGenerator.Generate();
        }
