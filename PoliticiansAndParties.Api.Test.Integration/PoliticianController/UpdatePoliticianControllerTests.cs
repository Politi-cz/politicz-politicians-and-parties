...
namespace PoliticiansAndParties.Api.Test.Integration.PoliticianController
{
    public class UpdatePoliticianControllerTests : IClassFixture<PoliticiansAndPartiesApiFactory>
    {
        ...
        public async Task UpdateAsync_UpdatesPolitician_WhenDataValid()
        {
            ...
            // Act
            var updatePoliticianResponse = await _client.PutAsJsonAsync($"api/political-parties/politician/{createdPolitician!.Id}", updatedPolitician);
            var getPoliticianResponse = await _client.GetAsync($"api/political-parties/politician/{createdPolitician!.Id}");

            // Assert
            updatePoliticianResponse.StatusCode.Should().Be(System.Net.HttpStatusCode.OK);
            var result = await updatePoliticianResponse.Content.ReadFromJsonAsync<PoliticianDto>();
            result.Should().BeEquivalentTo(updatedPolitician, options => options.Using<DateTime>(ctx => ctx.Subject.Should().BeCloseTo(ctx.Expectation, TimeSpan.FromMilliseconds(100))).WhenTypeIs<DateTime>());
            ...
            returnedPolitician.Should().BeEquivalentTo(result, options => options.Using<DateTime>(ctx => ctx.Subject.Should().BeCloseTo(ctx.Expectation, TimeSpan.FromMilliseconds(100))).WhenTypeIs<DateTime>());
        }
        ...
        public async Task UpdateAsync_ReturnsNotFound_WhenPoliticianNotFound()
        {
            ...
            // Act
            var updatePoliticianResponse = await _client.PutAsJsonAsync($"api/political-parties/politician/{nonExistingId}", updatedPolitician);
